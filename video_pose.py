...
input_filename = 'push_up.mp4'
cap = cv.VideoCapture(input_filename)
cap_width = int(cap.get(cv.CAP_PROP_FRAME_WIDTH))    
cap_hight = int(cap.get(cv.CAP_PROP_FRAME_HEIGHT))

output_filename = 'output.mp4'
fps = cap.get(cv.CAP_PROP_FPS)
output = cv.VideoWriter(output_filename,cv.VideoWriter_fourcc(*'MP4V'),fps,(cap_width,cap_hight))
...
def main():
    ...
    with mp_pose.Pose(
        ...
        # for idx,image in enumerate(video_array):
        while cap.isOpened():
            ...
            video_array.append(debug_image)
